...
import { ServerGenerator, ServerMode, ServerGeneratorOptions, GenerationResult } from './ServerGenerator.js';
...
export class BuildPipeline {
  ...
  constructor(config: BuildConfig) {
    this.config = {
      ...
      mode: config.mode || ServerMode.SIMPLE,
        ...
        if (result.validation.hasErrors) {
          ...
          for (const validationResult of validationResults) {
            if (!validationResult.isValid) {
              console.error(`  Tool: ${validationResult.tool.className} (${validationResult.tool.filePath})`);
              
              const errors = validationResult.issues.filter(i => i.severity === ValidationSeverity.ERROR);
