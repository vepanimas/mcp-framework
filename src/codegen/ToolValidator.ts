...
export class ToolValidator {
  ...
  private validateClassStructure(
    ...
    if (!extendsMCPTool) {
      const { line, character } = sourceFile.getLineAndCharacterOfPosition(classDeclaration.name?.getStart() || 0);
      issues.push({
        filePath: tool.filePath,
        line: line + 1,
        column: character + 1,
  ...
  private validateSchema(
    ...
    if (!schemaText.includes('z.object(') && !schemaText.includes('defineSchema(')) {
      const { line, character } = sourceFile.getLineAndCharacterOfPosition(schemaProperty.getStart());
      issues.push({
        filePath: tool.filePath,
        line: line + 1,
        column: character + 1,
  ...
  private checkDependencies(
    ...
    for (const importInfo of imports) {
      if (
        ...
        !allowedImports.some(allowed => importInfo.path === allowed || importInfo.path.startsWith(`${allowed}/`))
      ) {
        const { line, character } = sourceFile.getLineAndCharacterOfPosition(importInfo.node.getStart());
        issues.push({
          filePath: tool.filePath,
          line: line + 1,
          column: character + 1,
  ...
  private enforceBestPractices(
    ...
    if (executeMethod) {
      ...
      if (!executeText.includes('try') || !executeText.includes('catch')) {
        const { line, character } = sourceFile.getLineAndCharacterOfPosition(executeMethod.getStart());
        issues.push({
          filePath: tool.filePath,
          line: line + 1,
          column: character + 1,
   ...
   */
  private getLeadingCommentForNode(sourceFile: ts.SourceFile, nodeName: string): string | undefined {
