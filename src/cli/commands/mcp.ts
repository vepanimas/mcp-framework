  ...
  try {
    ...
    if (!description) {
      const response = await prompts({
        ...
        validate: (value) =>
          value.length >= 10 ? true : 'Description must be at least 10 characters',
      });
 ...
 * @param options Command options
 */
async function validateTools(options: { config?: string; fix?: boolean }): Promise<void> {
  try {
    // Load config
    const config = await loadConfig(options.config);
    ...
    const errorCount = validationResults.reduce(
      (count, result) =>
        count + result.issues.filter((i) => i.severity === ValidationSeverity.ERROR).length,
      0
    );

    const warningCount = validationResults.reduce(
      (count, result) =>
        count + result.issues.filter((i) => i.severity === ValidationSeverity.WARNING).length,
      0
    );

    const infoCount = validationResults.reduce(
      (count, result) =>
        count + result.issues.filter((i) => i.severity === ValidationSeverity.INFO).length,
      0
    );
 ...
 * @param options Command options
 */
async function migrateToSDK(options: { config?: string; backup?: boolean }): Promise<void> {
  try {
    // Load config
    const config = await loadConfig(options.config);
 ...
 * @param options Command options
 */
async function checkSDKCompatibility(options: { config?: string }): Promise<void> {
  try {
    // Load config
    const config = await loadConfig(options.config);
    ...
    for (const tool of discoveryResult.tools) {
      ...
      if (
        content.includes('this.jsonResponse(') ||
        content.includes('this.textResponse(') ||
        content.includes('this.errorResponse(')
      ) {
      ...
      if (
        content.includes('context.projectRoot') ||
        content.includes('context.workspaceFiles') ||
        content.includes('context.git')
      ) {
    ...
    console.error(
      `❌ SDK compatibility check failed: ${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
